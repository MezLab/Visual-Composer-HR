...
class HR_Module
{
  ...
  public function duplicate(int $id){
    ...
    $query_1 = $wpdb->insert( 'wpackage_hr_module', array(
      "code" => $query[0]['code'],
      "name" => $query[0]['name'] . '_copy_module_'. $query[0]['ID'],
  ...
  public function filterModule($text){
    ...
    $query = $wpdb->get_results("select name from wpackage_hr_module;", ARRAY_A);

    $select = "<option value=''>" . $text . "</option>";

    for ($i=0; $i < count($query); $i++) {

        $name = explode(" ", $query[$i]["name"]);
        $newName = "";
  ...
  public function replyModule($id){
    ...
    $this->reply_msg = $query[0]['reply_content'];
