  ...
  public function login(string $x, string $y, string $z){
    ...
    */
    ...
    if(!empty($x) && !empty($y)){
      ...
      header('Location: ' . url_my_path() . 'wp-admin/admin.php?page=wpackage_hr' . $r['message']);
    }else{
      header('Location: ' . url_my_path() . 'wp-admin/admin.php?page=wpackage_hr' . $err);
    }
  }
    ...
    try {
      switch ($request) {
        ...
        case 'logout': // Utente Disconnesso
          session_destroy();
          header('Location: ' . url_my_path() . 'wp-admin/admin.php?page=wpackage_hr&admin=noAccess');
          throw new Exception("Disconnessione avvenuta con successo");
            break;
        case 'loggin': // Utente connesso
