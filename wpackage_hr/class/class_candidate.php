...
class HR_Candidate
{
  ...
  public function sendMail($mail, $code, $id){
    ...
    for ($i=0; $i < strlen($query[0]['reply_content']); $i++) { 
      ...
      $stringa .= $query[0]['reply_content'][$i];
    } 
...
//--------------------------------------------
  ...
  public function download($ID){
    ...
    */
    ...
    //cartella generale
    $dir = plugin_dir_path(__DIR__) . "candidate/";
    ...
    if(file_exists($filename)){
      header("Location: ".url_my_plugin()."/admin/download.php?name=".$name."&filename=".$filename);
    }
  ...
  public function packet($dirPacket){
    ...
    */
    ...
    $dir_path = plugin_dir_path(__DIR__) . "candidate/" . $dirPacket; //indirizzo completo della cartella da zippare
