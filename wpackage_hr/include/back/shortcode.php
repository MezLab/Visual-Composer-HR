...
function WPackage_HR_Module($atts){
  ...
  $query = $wpdb->get_results("select name, content, deadline from wpackage_hr_module where ID=$ID ;", ARRAY_A);

  $name = explode(" ", $query[0]["name"]);
  $newName = "";
      ...
      for ($i=1; $i < count($arrayJson); $i++) {
        ...
        switch ($json['type']) {
          ...
          $shortCode .= "<div class='elem'>" ;
              ...
              if($json['required']){
                $shortCode .= "<p>Lingue?</p><select name='" . $json['name'] . "' required=''>" . $opt . "</select>";
              }else{
                $shortCode .= "<p>Lingue?</p><select name='" . $json['name'] . "'>" . $opt . "</select>";
              }
          $shortCode .= "</div>";
