...
function MB_Order(){
  ...
  for (let p = 0; p < MB_Size(); p++) {
    ...
    while (z < 2) {
      var string = "";
      for (let f = 0; f < spans[z].getAttribute('onclick').length; f++) {
        if (isNaN(parseInt(spans[z].getAttribute('onclick')[f]))){
          string += spans[z].getAttribute('onclick')[f];
        }else{
          string += String(p);
        }
      }
      spans[z].setAttribute('onclick', string);
      z++;
    }
  }
...
function parsForm(){
  ...
  for (let index = 0; index < singleMeta.length; index++) {
    ...
    objectInput = tempObjectInput; // Reset Object;
